...
if (!defined($ARGV[0])) {
	...
	print "\n";
	print "<ip_base>    - The first 2 octets of every address in the\n";
	print "               range to be looked up. (Eg. 127.0)\n";
...
while ($host < $last+1) {
	my $hostd = 1;
	while ($hostd < 256) {
		print "Looking up $base.$host.$hostd.... ";
		print "Waiting...";
	 	my $hostname = gethostbyaddr(inet_aton("$base.$host.$hostd"),AF_INET);
		if (!defined($hostname)) {
 			$hostname = "No RDNS";
 		}
